...
8. **Two exports, two files.** `/export` preview **is** the client brief: company, date, short engagement line, then only completed frameworks. If BMC is the only lock, that is the whole brief — do not leave empty slots for the other 15. BMC stays a nine-box on screen and on paper. Coverage, nav, and the two buttons are screen-only (`@media print` hides them). **Download for an agent** writes a flattened `.md` (headings + bullets, no JSON dump). **Print brief** calls `window.print()` — not a Download PDF button. The `.md` never becomes the preview. Do not put the 16-agent roster on the brief.
