...
import { clientDirectionFromInstruction, normalizeQuestions } from "@/lib/frameworkRunFollowUp";
import { isBizIntakeReady } from "@/lib/intake";
...
export function deriveActiveAgents(artifacts, buckets = {}) {
  const completed = key => artifactIsComplete(artifacts?.[key], key);
  const bmcReady = isBizIntakeReady(buckets?.biz);
  return ORDER.filter(key => {
    if (key === "bmc") return bmcReady;
    return SOURCES[key].length > 0 && SOURCES[key].every(completed);
  });
}
...
export function buildContextSnapshot(frameworkId, buckets, artifacts, instruction = "", chats = null) {
  ...
  // The current framework's own needs_input artifact is not a complete
  // upstream. Carry its questions so a directed follow-up can see what
  // was already asked instead of starting the same loop.
  const current = artifacts?.[frameworkId];
  const priorQuestions = current?.status === "needs_input"
    ? normalizeQuestions(current.nextQuestions)
    : [];
  const direction = clientDirectionFromInstruction(instruction);
  if (direction && priorQuestions.length) {
    userClarifications.push({
      frameworkId,
      agent: FW[frameworkId]?.role || frameworkId,
      statement: direction,
    });
  }
  ...
  const fingerprintInput = safeJSON({
    frameworkId, bucketSnapshots, directArtifacts, sharedArtifacts, legacyArtifacts,
    userClarifications, priorQuestions, instruction,
  });
