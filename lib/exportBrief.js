...
export function listCompleteFrameworks(artifacts = {}) {
  return ORDER.filter(id => artifactIsComplete(artifacts[id], id) || artifacts[id]?.status === "stale");
}

// Client brief order: the signed constraint leads. Each framework once.
export function orderBriefFrameworks(ids = []) {
  const seen = new Set();
  const unique = [];
  ids.forEach(id => {
  ...
  return [...unique.filter(id => id === "toc"), ...unique.filter(id => id !== "toc")];
}

export function listBriefFrameworks(artifacts = {}) {
  return orderBriefFrameworks(listCompleteFrameworks(artifacts));
}
...
// Chrome will not start a download from a detached <a download>.
// It copies blob bytes after click(), asynchronously. Revoking in the same
// turn — or on the next tick (setTimeout 0) — still races that copy: the
// shelf shows the blob UUID, then Deleted, and agentDownloadName never lands.
const AGENT_DOWNLOAD_REVOKE_MS = 40_000;

export function downloadAgentFile(state) {
  ...
  setTimeout(() => URL.revokeObjectURL(url), AGENT_DOWNLOAD_REVOKE_MS);
}
