...
// Every property is required unless listed in `optional` (or `required` is
// passed explicitly). playerSchema used to hand-roll that list so old maps
// without `url` still validated; this is that pattern as a helper.
export function objectSchema(properties, options = {}) {
  const optional = new Set(options.optional || []);
  const required = options.required || Object.keys(properties).filter(key => !optional.has(key));
  return {
    type: "object",
...
const claimSchema = (extra = {}) => objectSchema({
  ...
  ...extra,
  ...groundingProperties,
  // Optional flag: the renderer lights at most one lamp (first-wins).
  feedsForward: { type: "boolean" },
}, { optional: ["feedsForward"] });
...
const LOGO_STATUS = ["unfetched", "ok", "missing", "rejected"];
const LOGO_SOURCE = ["favicon", "og:image", "apple-touch-icon"];

const playerSchema = objectSchema({
  name: stringSchema(),
  ...
  url: nullableString(),
  // Schema-only until Phase 3a fetches marks. Absent on old players.
  logoUrl: nullableString(),
  logoStatus: enumSchema(LOGO_STATUS),
  logoFetchedAt: nullableString(),
  logoSource: enumSchema(LOGO_SOURCE),
  ...groundingProperties,
}, { optional: ["url", "logoUrl", "logoStatus", "logoFetchedAt", "logoSource"] });

const verdictSchema = objectSchema({
  headline: stringSchema(),
  subline: stringSchema(),
  grounding: objectSchema(groundingProperties),
});
...
const definitions = {
  bmc: {
    ...
    payloadTemplate: {
      ...
      // Empty string, never a made-up thesis. Renderer treats "" as absent.
      readout: "",
    },
  ...
  industrymap: {
    ...
    payloadSchema: objectSchema({ map: objectSchema({
      ...
      // Optional survey thesis. Absent on old maps; do not invent one.
...
function normalizeGrounding(value) {
  const source = value && typeof value === "object" && !Array.isArray(value) ? value : {};
  return {
    basis: BASIS.includes(source.basis) ? source.basis : "missing",
    confidence: CONFIDENCE.includes(source.confidence) ? source.confidence : "low",
    evidenceRefs: Array.isArray(source.evidenceRefs) ? clone(source.evidenceRefs) : [],
  };
}

// Fill missing optional fields with empty/undefined. Never invent a thesis,
// logo, or feeds-forward flag.
function backfillOptionalPayload(frameworkId, payload) {
  const next = payload && typeof payload === "object" && !Array.isArray(payload) ? clone(payload) : {};
  if (frameworkId === "bmc") {
    next.readout = emptyString(next.readout);
  }
  if (frameworkId === "industrymap" && next.map && typeof next.map === "object" && !Array.isArray(next.map)) {
    if (next.map.verdict != null && typeof next.map.verdict === "object" && !Array.isArray(next.map.verdict)) {
      const raw = next.map.verdict;
      next.map.verdict = {
        headline: emptyString(raw.headline),
        subline: emptyString(raw.subline),
...
function walkClaims(value, visit) {
  if (Array.isArray(value)) {
    value.forEach(item => walkClaims(item, visit));
    return;
  }
  if (!value || typeof value !== "object") return;
  // A claim is an object that carries finding text (and maybe feedsForward).
  if (typeof value.text === "string" || Object.prototype.hasOwnProperty.call(value, "feedsForward")) {
    visit(value);
  }
  Object.values(value).forEach(child => walkClaims(child, visit));
}

// Renderer helper: the first feedsForward:true claim wins. Schema allows more
// than one; do not light a second lamp.
export function firstFeedsForwardClaim(artifact) {
  let winner = null;
  walkClaims(artifact, claim => {
    if (winner || claim.feedsForward !== true) return;
    winner = claim;
  });
  return winner;
}
...
export function normalizeFrameworkArtifact(value, expectedFrameworkId) {
  ...
  return {
    ...
    payload: backfillOptionalPayload(inferredId, mergeTemplate(definition.payloadTemplate, source.payload)),
