...
// ?new=1 only opens intake — Back to the canvas still works.
// The prior map is dropped when Draw commits a new company, or when
// ?demo= actually changes buckets. Marking stale is not enough: ToC
// and /export would still show the old constraint text.
export function shouldClearPriorCompany({ bucketsChanged = false, committingDraw = false } = {}) {
  return Boolean(bucketsChanged || committingDraw);
}
