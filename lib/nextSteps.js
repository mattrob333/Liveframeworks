...
// The filled canvas ships ONLY one next move (or Export on RACI): the next
// chapter in ORDER. A completed run has no ready feed — every feed is already
// done — so do not wait on recommended. Unfilled maps stay quiet.
...
export function resolveNextMove({ frameworkId, artifacts = {}, buckets = {} } = {}) {
  ...
  // Filled map (BMC, Industry Map, Five Forces, SWOT, Three Horizons, Blue
  // Ocean, 7S, and the rest): the next chapter in the waterfall. Asking
  // recommended first hides this on a 16/16 run — every feed is already done.
  if (mapIsFilled(artifacts[frameworkId], frameworkId)) {
    return moveFor(ORDER[orderIndex(frameworkId) + 1]);
  }

  const { recommended } = resolveNextSteps({ frameworkId, artifacts, buckets });
  return recommended ? moveFor(recommended) : null;
}
