...
// recommended ready feed, or — when this map is filled and no feed is ready —
// the earliest incomplete feed. Same line + button as BMC → Industry Map.
...
export function resolveNextMove({ frameworkId, artifacts = {}, buckets = {} } = {}) {
  ...
  const { recommended, steps } = resolveNextSteps({ frameworkId, artifacts, buckets });
  if (recommended) return moveFor(recommended);

  // Filled map, no ready feed (Five Forces, SWOT, Three Horizons, Blue Ocean, 7S
  // in sequential waterfall): still one next move, from the feeds list.
  if (!mapIsFilled(artifacts[frameworkId], frameworkId)) return null;
  const fallback = steps.find(step => step.status !== "done");
  return moveFor(fallback?.id);
}
