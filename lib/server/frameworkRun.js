...
// Shared with BMC readout and Industry Map verdict. Do not ask the model to
// invent a thesis when the evidence does not support one.
export const EMPTY_THESIS_FALLBACK_RULE = "An absent thesis beats a hollow one. If the evidence does not support a real thesis, omit the field or leave it empty. Do not invent a thesis.";

function thesisPrompt(frameworkId) {
  if (frameworkId === "bmc") {
    return `\n\nWrite payload.readout as one sentence that states the canvas thesis — how this business makes, delivers, and keeps money — only when the evidence supports a real thesis. ${EMPTY_THESIS_FALLBACK_RULE}`;
  }
  if (frameworkId === "industrymap") {
    return `\n\nFor each player, set url to that company's official site when the research brief or source registry already has a real http(s) URL for them. Do not invent or guess domains.\n\nWrite map.verdict {headline, subline, grounding} as the industry-survey thesis only when the evidence supports one. ${EMPTY_THESIS_FALLBACK_RULE}\n\nAt most one claim may set feedsForward to true (the flow that feeds the later constraint diagnosis). If none is clear, omit it.`;
  }
  return "";
}
