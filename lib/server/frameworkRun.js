...
const MAX_RESEARCH_PAUSE_TURNS = 8;
const MAX_WEB_SEARCHES = 5;
const RESEARCH_MAX_TOKENS = 6_000;
const SYNTHESIS_MAX_TOKENS = 10_000;
...
export async function parseFrameworkRunRequest(req) {
  ...
  if (!apiKey) {
    throw new FrameworkRunError("Missing Anthropic API key. Add yours in Settings.", {
      status: 400,
...
async function callAnthropic({ apiKey, body, clientSignal, deadline }) {
  ...
  try {
    ...
    if (!response.ok) throw mapProviderError(response.status, data, response.headers);
...
Research the named company and its official site first when a company name or URL is available. Then seek relevant external market, customer, competitor, regulatory, or operating evidence for this framework. Distinguish company-provided evidence from web evidence and from inference. Cite every web-derived claim using the web search citations provided by the tool.
...
async function runResearch({ apiKey, frameworkId, instruction, contextJson, clientSignal, deadline }) {
  ...
  for (let attempt = 0; attempt < MAX_RESEARCH_PAUSE_TURNS; attempt += 1) {
    ...
    if (stopReason === "max_tokens") {
      throw new FrameworkRunError("The research phase reached its output limit before completing.", {
        status: 502,
  ...
  const text = providerTurns.map(turn => textFromContent(turn.content)).filter(Boolean).join("\n\n").trim();
  if (!text) {
    throw new FrameworkRunError("The research phase completed without a usable evidence brief.", {
      status: 502,
      code: "empty_research",
      retryable: true,
    });
...
Create one complete framework artifact that exactly follows the supplied JSON schema. Use only the engagement context, research brief, source registry, and clearly labeled inference. Do not browse in this phase. Do not place markdown fences around the JSON.
...
function finalizeArtifact(candidate, frameworkId, now) {
  ...
  const hasQuestions = Boolean((completion.artifact?.nextQuestions || []).some(question => String(question || "").trim()));
  if (!integrityErrors.length && missingContentErrors.length && hasQuestions) {
...
async function requestSynthesis({ apiKey, frameworkId, messages, clientSignal, deadline, maxTokens = SYNTHESIS_MAX_TOKENS }) {
  return callAnthropic({
    ...
    body: {
      ...
      output_config: {
        format: {
          type: "json_schema",
          schema: artifactSchema(frameworkId),
        },
      },
    },
  });
...
async function runSynthesis({ apiKey, frameworkId, instruction, contextJson, research, clientSignal, deadline }) {
  ...
  const first = await requestSynthesis({ apiKey, frameworkId, messages: initialMessages, clientSignal, deadline });
  ...
  if (first.data.stop_reason === "max_tokens") {
    throw new FrameworkRunError("The framework artifact reached its output limit before completing.", {
      status: 502,
      code: "synthesis_incomplete",
      retryable: true,
    });
  }
  if (first.data.stop_reason !== "end_turn") {
  ...
  try {
    ...
    finalized = finalizeArtifact(parsed, frameworkId, now);
  ...
  const repair = await requestSynthesis({ apiKey, frameworkId, messages: repairMessages, clientSignal, deadline });
  ...
  const repaired = finalizeArtifact(repairedCandidate, frameworkId, now);
...
export async function runFrameworkGeneration({ apiKey, frameworkId, instruction, contextJson, clientSignal, requestId, runId }) {
  const startedAt = Date.now();
  const deadline = startedAt + RUN_BUDGET_MS;
  console.log("[api/framework-run] start", { requestId, runId, frameworkId, model: MODEL });

  const research = await runResearch({ apiKey, frameworkId, instruction, contextJson, clientSignal, deadline });
