...
export const hasApiKey = () => validateApiKey(getKey()).ok;
...
// New company keeps the API key and intake buckets. The prior company's
// map lives in lf:artifact:*, lf:runs:*, lf:output:*, lf:chat:*,
// lf:selected:*, and lf:active — those must not carry into the next firm.
export const clearCompanyWork = () => safe(() => {
  Object.keys(localStorage)
    .filter(key => (
  ...
  if (typeof window !== "undefined") {
    window.dispatchEvent(new CustomEvent("lf:storage", { detail: { key: PREFIX + "artifact:*" } }));
  }
  return { ok: true };
}, { ok: false, error: "Browser storage could not be cleared." });
