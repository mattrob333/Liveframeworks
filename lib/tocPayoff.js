...
// orgInstallForLoadedCompany picks the file for the loaded hostname.
// TocRoster mounts when that glance parse has teams. Empty teams
// means this company has no signed install — omit the roster.
export function orgInstallForLoadedCompany(installs, hostname) {
  const host = String(hostname || "").trim().toLowerCase();
  if (!host || !installs || typeof installs !== "object") return "";
  for (const pack of Object.values(DEMO_PACKS)) {
    if (!host.includes(pack.company)) continue;
