...
test("second run with the ignore-answer direction does not repeat the identical question loop", () => {
  ...
  assert.match(decision.reason, /not to repeat those questions/i);
...
test("a distinct follow-up question is still needs_input", () => {
  const decision = resolveNeedsInputDecision({
    instruction: ANSOFF_IGNORE_ANSWER_DIRECTION,
    priorQuestions: FIRST_QUESTIONS,
    nextQuestions: ["Which named account is the Northgate pilot still waiting on?"],
    missingContent: true,
  });

  assert.equal(decision.status, "needs_input");
  assert.deepEqual(decision.questions, ["Which named account is the Northgate pilot still waiting on?"]);
});
